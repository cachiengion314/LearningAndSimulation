...
public struct Square
{
  ...
  public void InitSqaure(Vector2 position, float gridScale)
  {
    this.position = position;

    topRight = position + Vector2.one / 2f * gridScale;
  ...
  {
    var topLerp = Mathf.InverseLerp(values[3], values[0], isoValue);
    topLerp = Mathf.Clamp01(topLerp);
    topCenter = topLeft + (topRight - topLeft) * topLerp;

    var rightLerp = Mathf.InverseLerp(values[0], values[1], isoValue);
    rightLerp = Mathf.Clamp01(rightLerp);
    rightCenter = topRight + (bottomRight - topRight) * rightLerp;

    var bottomLerp = Mathf.InverseLerp(values[2], values[1], isoValue);
    bottomLerp = Mathf.Clamp01(bottomLerp);
    bottomCenter = bottomLeft + (bottomRight - bottomLeft) * bottomLerp;

    var leftLerrp = Mathf.InverseLerp(values[3], values[2], isoValue);
    leftLerrp = Mathf.Clamp01(leftLerrp);
    leftCenter = topLeft + (bottomLeft - topLeft) * leftLerrp;
  }
  ...
  void Triangulate(int configuration)
  {
    switch (configuration)
    {
      ...
      case 9:
        ...
        break;
      case 10:
        vertices.AddRange(new Vector3[] { rightCenter, bottomRight, bottomCenter, leftCenter, topLeft });
        triangles.AddRange(new int[] { 0, 1, 2, 0, 2, 3, 0, 3, 4, 0, 4, 5 });
        uvs.AddRange(new Vector2[] { rightCenter, bottomRight, bottomCenter, leftCenter, topLeft });
        break;
      case 11:

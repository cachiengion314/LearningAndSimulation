...
// My thought in Unity's choice: 
// They picked the method that actually works for persistent data, instead of hammering users with an “official but wrong” RenderGraph pattern (RenderGraph resources only live within a single frame)
// They intentionally demonstrate that persistent history must come from manually allocated RTHandles, not RenderGraph itself (by inject m_OldFrameHandle)
// But it reveals a deeper flaw in URP RG design
// URP’s RenderGraph system does not yet unify persistent resource management.
// HDRP handles this more elegantly.
...
public class KeepFrameFeature : ScriptableRendererFeature
{
    ...
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        ...
        RenderingUtils.ReAllocateHandleIfNeeded(
            ref m_OldFrameHandle, descriptor, FilterMode.Bilinear, TextureWrapMode.Clamp, name: textureName
        );
